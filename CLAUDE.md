...
| Uploads bucket needs S3 CORS for browser uploads | `cors: [{ allowedMethods:[PUT,POST,GET,HEAD], allowedOrigins:['*'], allowedHeaders:['*'] }]` on iac-uploads in foundation-stack.ts | The dashboard uploads via a cross-origin presigned PUT (XHR) straight to S3, so the browser fires a CORS **preflight (OPTIONS)** first. With no bucket CORS config S3 returns `NoSuchCORSConfiguration` → the upload FAILS in the UI ("upload failure error") — even though curl/Python uploads succeed (they don't preflight). AllowedOrigins `*` is safe: the time-limited single-key presigned URL is the real auth boundary, and `*` keeps uploads working across dashboard origins (API GW / future CloudFront / localhost). Verified live: preflight → HTTP 200, Access-Control-Allow-Methods PUT. |
